...
const Blog = () => {
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8080/api/diplomWork/blog",
        formData,
        {
          headers: {
  ...
  const handleDeleteSlider = async (sliderId) => {
    try {
      const response = await axios.delete(`http://localhost:8080/api/diplomWork/blog/${sliderId}`);
  ...
  const handleModalOk = async () => {
    ...
    try {
      const response = await axios.put(
        `http://localhost:8080/api/diplomWork/blog/${currentSlider._id}`,
        formData
      );
